...
//  const [dog, setDog] = useState(null)
  ...
  return (
    <main className="p-3 max-w-4xl mx-auto my-auto">
      <h1 className="text-3xl font-semibold text-center my-9">Update your dog info</h1>
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="flex flex-col gap-2">
          ...
          <input type="text" placeholder="e.g.: Herding" className="border p-3 mb-4 rounded-lg" name='breed_group' onChange={handleChange} value={formData?.breed_group} />
          <label htmlFor="life_span" className="font-semibold text-slate-600">Life span (yr/yrs): <span className='font-normal text-slate-500'>(e.g.: Enter average life span or range (min-max))</span></label>
          <input type="text" placeholder="e.g.: 5 - 12 years" className="border p-3 mb-4 rounded-lg" name='life_span' maxLength='62' onChange={handleChange} value={formData?.life_span} />
          <label htmlFor="origin" className="font-semibold text-slate-600">Country of origin: <span className='font-normal text-slate-500'></span></label>
          <input type="text" placeholder="e.g.: Spain and Portugal" className="border p-3 mb-4 rounded-lg" name='origin' onChange={handleChange} value={formData?.origin} />
        </div>
        <div className="flex flex-col gap-2">
          <label htmlFor="description" className="font-semibold text-slate-600">Description: <span className='font-normal text-slate-500 '></span></label>
          <textarea type="text" placeholder="e.g.: The dewclaws are never removed and the feet are cat-like." className="border p-3 mb-4 rounded-lg" name='description' onChange={handleChange} value={formData?.description} />
          <label htmlFor="temperament" className="font-semibold text-slate-600">Select 1 or more temperaments: <span className='font-normal text-slate-500'>(CTRL + click to select more than one option)</span></label>
          <select multiple className='text-slate-600 mb-4' onChange={handleTemperamentChange}>
          ...
          }
          {
            selectedTemperaments.length > 0
              ? (<p className="font-semibold text-green-700 ">
                {selectedTemperaments?.length > 0 ? "Temperaments Replaced: " : ''} <span className="font-normal text-green-600">{selectedTemperaments?.join(', ')}</span></p>)
              : ''
          }
          <div className="flex flex-col gap-3 flex-1 md:mt-0">
            <label htmlFor="images" className="font-semibold text-slate-600">Upload 1 o more images: <span className='font-normal text-slate-500'>The first image will be the cover (max 3)</span></label>
            <div className="flex">
        ...
        <button disabled={loading || uploading} className="col-span-2 bg-slate-600 text-white p-3 rounded-lg uppercase hover:opacity-90 disabled:opacity-80">
          {
            loading ? 'Loading...' : 'Update Dog'
          }
        </button>
        {
          error && <p className='col-span-2 mt-3 text-red-500 text-sm flex justify-center'>{error}</p>
        }
        <p className='col-span-2 text-green-800 mt-5 flex justify-center '>{updateSuccess ? 'Dog updated successfully!' : ''}</p>
        <ul className='col-span-2 flex justify-center gap-4'>
          ...
          <li className='bg-slate-500 text-white rounded-lg p-2 hover:opacity-95'><Link to={`/dog/${params.dogId}`}>View Details</Link></li>
        </ul>
      </form>
    </main>
  )
